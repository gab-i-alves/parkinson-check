...
from api.schemas.binding import BindingDoctor, BindingPatient, BindingRequestResponse, SentBindingRequestResponse
from core.models.users import Doctor
from core.services import notification_service
from core.models import User, Bind, Patient
from sqlalchemy.orm import Session
from fastapi import HTTPException
from http import HTTPStatus
from core.enums import BindEnum, UserType, NotificationType

def get_pending_bind_requests(user: User, session: Session) -> list[BindingRequestResponse]:
    if user.user_type == UserType.DOCTOR:
        bindings = session.query(Bind).filter(
...
def send_bind_request(user: User, session: Session, user_to_bind: int) -> Bind:
    if user.id == user_to_bind:
        raise HTTPException(HTTPStatus.BAD_REQUEST, detail="Não é possível vincular-se a si mesmo.")
    ...
    if inactive_bind:
        ...
        if user.user_type == target_user.user_type:
            raise HTTPException(HTTPStatus.BAD_REQUEST, detail="Médicos só podem vincular pacientes e vice-versa.")
...
def accept_bind_request(user: User, session: Session, bind_id: int) -> Bind:
    ...
    if bind_to_accept.status != BindEnum.PENDING:
        raise HTTPException(HTTPStatus.BAD_REQUEST, detail="Apenas solicitações pendentes podem ser aceitas.")
...
def reject_bind_request(user: User, session: Session, bind_id: int):
    ...
    if bind_to_reject.status != BindEnum.PENDING:
        raise HTTPException(HTTPStatus.BAD_REQUEST, detail="Apenas solicitações pendentes podem ser rejeitadas.")
...
def unbind_users(user: User, session: Session, bind_id: int):
    ...
    if bind_to_reverse.status != BindEnum.ACTIVE:
        raise HTTPException(HTTPStatus.BAD_REQUEST, detail="Apenas vínculos ativos podem ser desfeitos.")
