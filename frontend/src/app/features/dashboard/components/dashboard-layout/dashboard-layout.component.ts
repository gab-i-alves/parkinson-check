...
export class DashboardLayoutComponent implements OnInit, OnDestroy {
  ...
  getDisplayName(fullName: string, role: string): string {
    if (role === 'medico') {
      // Para médicos, mostrar os dois primeiros nomes
    ...
    // Para outros usuários, mostrar apenas o primeiro nome
  ...
  private updateBreadcrumbs(): void {
    ...
    if (matchingKey && breadcrumbMap[matchingKey]) {
      ...
      if (overrideLabel && this.breadcrumbs.length > 0) {
        // Update the last breadcrumb with the override label
